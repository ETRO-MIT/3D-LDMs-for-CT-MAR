...
- **Stage 2 (Anatomy-Conditioned LDM): `paired_train.csv`**
  Pairs the synthetic artifacted CT with the clean ground-truth target:
  ```csv
...
- **Stage 2 (Anatomy + Metadata-Conditioned LDM): `paired_metadata_train.csv`**
  Requires the path to each case's sidecar metadata `.json` file (generated by `Main.py generate` containing `region`, `side`, and `metal_name`):
  ```csv
  synthetic_image_path,implant_only_image_path,metadata_path
  /path/to/synth_case_001.nii.gz,/path/to/implant_only_case_001.nii.gz,/path/to/metadata_case_001.json
  /path/to/synth_case_002.nii.gz,/path/to/implant_only_case_002.nii.gz,/path/to/metadata_case_002.json
  ```
